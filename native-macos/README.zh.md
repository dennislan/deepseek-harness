...
同一次检查也可在应用菜单中手动触发。**检查更新…** 只在有内容可说时才在窗口右上角的提示
卡片上作答：检查中不弹任何提示；发现新版本时显示 `发现新版本 <version>，是否更新？`
（蓝色下载图标），并在整个安装期间保持——安装步骤只写日志，不再覆盖这条询问；结束或已是
最新时以绿色对号给出结果（`当前已是最新版本`、`<version> 已下载完成，下次启动应用时生效`），
失败则显示带原因与日志目录的刷新图标提示。若在一次运行尚未结束时点击，则显示该运行正在
进行的步骤；版本若已暂存，则直接汇报结果而不再安装一遍。提示卡不接收点击、不抢焦点，
因此手动检查同样不会阻塞操作。
