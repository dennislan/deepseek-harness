...
export function apply(_ctx: unknown): void {
      ...
      try {
        ...
        if (data.status === 'confirmed' && data.user) {
          ...
          if (loginRes.ok) {
            ...
            if (d.user) store.setState({ user: { id: d.user.id, displayName: d.user.displayName } })
          }
        }
    ...
    el.innerHTML = /* html */ `
      ...
      <div class="dsh-brand" aria-hidden="true">
        ...
        <div class="dsh-brand-content">
          ...
          <h1 class="dsh-brand-title">运行有迹可循</h1>
          <p class="dsh-brand-sub">模型、工具、技能、会话、沙箱、存储、循环——所有 Agent 能力均由插件组合。</p>
          <div class="dsh-brand-divider"></div>
          <p class="dsh-brand-tagline">Agent Workspace · by Dennis</p>
        </div>
      ...
      <div class="dsh-form-panel">
        ...
        <div class="dsh-topbar">
          <div>
            ...
            <p class="dsh-subtitle">Sign in to continue</p>
          </div>
  ...
  function renderOverlay(): void {
    ...
    if (subEl) subEl.textContent = snap.mode === 'wechat' ? 'Scan to sign in' : 'Sign in to continue'
